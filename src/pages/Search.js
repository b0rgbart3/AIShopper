...
function Search() {
  ...
  let user = state.user;
  //console.log("In the search page, user =", user);
  ...
  function analyze(url) {
    ...
    API.checkIfUrlWasAlreadyAnalyzed(url)
          ...
          API.extractUrl(state.currentSearch.image_url)
            .then((res) => {
              ...
              if (res.data.items.length > 0) {
                ...
                console.log(
                  "Google Image AI wasn't able to extract any items."
                );
              }
          ...
          API.getItems(searchId)
            .then((items) => {
              ...
              dispatch({
                type: ADD_SEARCH_DETAIL,
                newSearch: {
                  image_url: existingSearches.data[0].image_url,
                  items: items.data,
                },
              });
            })
    ...
    <div className="search">
      <div className="">
        ...
        {localSearches.length > 0 ? (
          ...
          <div>
            <p>No previous search images were found.</p>
          </div>
        )}
      </div>
      <hr />
      ...
      {state.has_url ? (
        <div className="analyze">
          ...
          <button
            ...
            onClick={() => {
              analyze(state.currentSearch.image_url);
            }}
          >
            Analyze this image
          </button>
